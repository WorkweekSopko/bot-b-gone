...
def classify_opens(opens_df, data, subscriber_col='subscriber_id'):
    ...
    is_community = pd.Series(pids).isin(data['community_set']).values  # feature/flag only (V10: not a verdict override)
    ...
    # Layer 4: Community signal (V10)
    # Community membership (is_community) is retained as a high-trust FEATURE,
    # but is no longer a blanket verdict override. A static allow-list was
    # auto-passing automated traffic generated by members' own corporate
    # security scanners, inflating engagement. Members' events are now screened
    # behaviorally like everyone else; is_community stays computed and
    # available for downstream use, but it no longer forces a "human" verdict.
    return mpp_confirmed | ml_human
...
def classify_clicks(clicks_df, data, subscriber_col='subscriber_id'):
    ...
    is_community = pids.isin(data['community_set']).values  # feature/flag only (V10: not a verdict override)
    ...
    # Step 1: Known human
    # V10: community membership is no longer a blanket pass here; members'
    # clicks are screened behaviorally like everyone else. is_community is kept
    # as a feature/flag (computed below), not a verdict override.
    known_human = (
...
def main():
    ...
    parser.add_argument('--community', help='Community verified humans CSV (optional; loaded as an informational feature — V10 no longer overrides the verdict on membership)')
    ...
    print(f"  Bots: {len(data['bot_set']):,}, Humans: {len(data['human_set']):,}, Community (informational): {len(data['community_set']):,}")
