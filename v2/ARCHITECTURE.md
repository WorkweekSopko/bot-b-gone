...
### Layer 4: Community Signal (feature, not override)

Subscribers with ground-truth human signals — paid subscribers / members, event registrants or attendees, community contributors, verified purchasers — carry a high-trust **membership flag**.

In earlier versions this flag was a blanket override: any event from a member was forced to "human." We removed that. **Membership is now a feature, not a verdict.** A static allow-list quietly auto-passed a large volume of automated traffic generated by members' own corporate security scanners — the engagement looked real but wasn't, and it inflated the numbers.

**Why keep the signal at all**: ~13% of verified humans have bot-like email patterns precisely because their corporate scanners create that activity. Membership tells you a *person* is behind the address; `is_community` stays computed and available for downstream use, but the cascade no longer uses it to decide the verdict — each event is screened behaviorally. A member whose only activity in a send is a sub-second scanner pre-click is still filtered.

## Click Classification (3 Steps)

### Step 1: Known Human (Protected)

These clicks bypass the models — they are protected by deterministic human signals:

> **V10 note:** community membership is no longer a protected row here. It is a high-trust *feature*, not a never-filter signal — member clicks are screened behaviorally like everyone else.
