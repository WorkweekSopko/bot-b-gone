...
**Why keep the signal at all**: ~13% of verified humans have bot-like email patterns precisely because their corporate scanners create that activity. Membership tells you a *person* is behind the address, so the model can weight borderline cases more generously — but each event is still screened behaviorally. A member whose only activity in a send is a sub-second scanner pre-click is still filtered.
